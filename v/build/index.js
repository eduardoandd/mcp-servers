...
const getServer = () => {
    ...
    server.registerTool("greet", {
        ...
        inputSchema: {
            name: v4_1.z.string().describe("Nome para cumpimentar")
        }
    }, async ({ name }) => {
        return {
            content: [
    ...
    server.registerTool("multi-greet", {
        ...
        inputSchema: {
            name: v4_1.z.string().describe("Nome para cumprimentar")
        },
        annotations: {
    ...
    server.registerTool('collect-user-info', {
        ...
        inputSchema: {
            infoType: v4_1.z.enum(["contact", "preferences", "feedback"])
        }
    ...
    server.experimental.tasks.registerToolTask("collect-user-info-task", {
        ...
        inputSchema: {
            infoType: v4_1.z.enum(['contact', 'preferences']).describe("Tipo de informações coletadas").default("contact")
        }
    }, {
    ...
    server.registerPrompt("greeting-template", {
        ...
        argsSchema: {
            name: v4_1.z.string().describe("Nome para incluir na saudação")
        }
    }, async ({ name }) => {
        return {
            messages: [
    ...
    server.registerTool('start-notification-stream', {
        ...
        inputSchema: {
            interval: v4_1.z.number().describe("Intervalo em milisegundos entre notificações").default(100),
            count: v4_1.z.number().describe("Número de notificações a enviar (0 a 100)").default(50)
        }
