...
server.registerTool("get_alerts", {
    ...
    const alertsData = await getWeather(alertsUrl);
    if (!alertsData) {
        return {
            content: [
                {
...
server.registerTool("get_forecast", {
    ...
    const pointsData = await getWeather(pointsUrl);
    if (!pointsData) {
        return {
            content: [
                {
    ...
    const forecastData = await getWeather(forecastUrl);
    if (!forecastData) {
        return {
            content: [
                {
